<script lang="ts">
	...
	// reveal.js's initialize() is async internally (event bindings are wired up after the promise
	// resolves); calling sync() before it resolves throws (unbind() forEach on an unset array), so
	// track readiness explicitly instead of racing it from the $effect below.
	...
	$effect(() => {
		slides
		if (!isEditing && deckReady) deck?.sync()
	})
	...
	const downloadPptx = async () => {
		if (!slidesMarkdown || isExporting) return
		isExporting = true
		errorMessage = ""
		try {
			const res = await fetch("/api/canvas/presentation/export", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ slides: slidesMarkdown })
			})
			if (!res.ok) {
				const err = await res.json().catch(() => ({}))
				throw new Error((err as { message?: string }).message ?? `HTTP ${res.status}`)
			...
			errorMessage = e instanceof Error ? e.message : "Kunne ikke laste ned presentasjonen"
		} finally {
			isExporting = false
		}
	}
...
<div class="document-page">
	<div class="canvas-topbar">
		...
		<div class="topbar-actions">
			...
			<button onclick={downloadPptx} disabled={!slidesMarkdown || isExporting} title="Last ned som PowerPoint">
				<span class="material-symbols-outlined">download</span>
				{isExporting ? "Genererer…" : "Last ned"}
			</button>
		</div>
	</div>
	...
	<div class="canvas-body">
		<div class="canvas-content">
			<!--
				Both panes stay mounted at all times and are toggled with CSS, rather than an
				{#if}/{:else}, because reveal.js's instance (`deck`) is bound to the specific
				`.reveal` DOM node it was constructed against. Destroying and recreating that
				node on every edit/preview toggle would leave `deck` pointing at a detached
				element, and the newly created node would never be initialized - so the
