...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
	...
	// this route never builds an agentic tool-calling stream at all (see the module comment above
	// the RAG block) - this is defense in depth so a future change to this file can't accidentally
	// forward an mcp tool to a vendor SDK that doesn't understand it. SharePoint MCP is deliberately
	// not offered to anonymous, unauthenticated embed visitors: unlike RAG libraries (curated,
	// presumed vetted for public-safe content), the MCP server is a live connection to a specific
	// internal SharePoint site, and the accessGroups RBAC that scopes MCP exposure elsewhere doesn't
	// apply on this route.
