...
import type { Agent, DBAgentUpdateInput } from "$lib/types/agents"

const validAgentUpdateInput: DBAgentUpdateInput = {
...
const notSupportedModelAgentUpdateInput: DBAgentUpdateInput = {
	config: {
		type: "",
...
describe("server route PATCH api/agents/[agentId]/+server", () => {
	...
	it("returns 201 and agent when user can create agents", async () => {
		...
		expect(updatedAgentResponse.agent.config.model).toBe(validAgentUpdateInput.config.)
	})
})
